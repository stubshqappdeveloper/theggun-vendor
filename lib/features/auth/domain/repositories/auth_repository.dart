...
class AuthRepository implements AuthRepositoryInterface {
  ...
  Future<ApiResponse> registration(
    ...
    if (businessLogo != null) {
      ...
      var part = http.MultipartFile('business_registration',
          businessLogo.readAsBytes().asStream(), list.length,
          filename: basename(businessLogo.path));
      request.files.add(part);
    }

    String ch = "00:00";
    if (registerModel.consultationHours != null) {
      if (registerModel.consultationHours!.trim().length > 1) {
        ch = "${registerModel.consultationHours!.trim()}:00";
      } else {
        ch = "0${registerModel.consultationHours!.trim()}:00";
      }
    }
