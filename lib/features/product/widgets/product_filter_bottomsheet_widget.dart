            ...
            Expanded(child: SizedBox(child: SingleChildScrollView(child: Column(
              ...
              children: [
                ...
                if(productProvider.selectedProductType != ProductTypeEnum.digital)...[
                  ...
                  Consumer<AddProductController>(builder: (context, addProductProvider, _) {
                    return Container(
                      height: productProvider.brandSeeMore || (addProductProvider.brandList?.length ?? 0) < 4 ? null : 150,
                      padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeMedium),
                      child: ListView.builder(
                        itemCount: addProductProvider.brandList?.length ?? 0,
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        itemBuilder: (context, index){
                          if(addProductProvider.brandList?[index].id == null) return const SizedBox.shrink();

                          return _FilterItem(
                            title: addProductProvider.brandList?[index].name,
                            checked: productProvider.selectedBrandIds.contains(addProductProvider.brandList?[index].id),
                            onTap: () {
                              _onCloseKeyboard(minPriceController, maxPriceController);
                              productProvider.onChangeBrandIds(addProductProvider.brandList![index].id!);
                            },
                          );
                        },
                      ),
                  ...
                  _ViewMoreWidget(
                    ...
                    isActive: (Provider.of<AddProductController>(context, listen: false).brandList?.length ?? 0) > 3,
                  ),
                  const SizedBox(height: Dimensions.paddingSizeMedium,),
                ...
                _ViewMoreWidget(
                  ...
                  isActive: (Provider.of<AddProductController>(context, listen: false).categoryList?.length ?? 0) > 3,
                ),
                const SizedBox(height: Dimensions.paddingSizeMedium,),
...
class _PublisherFilterItemWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ProductController>(builder: (context, productController, _) {
        return Consumer<AddProductController>(builder: (context, addProductController, _) {
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeExtraSmall),
                child: Container(
                  height: productController.brandSeeMore ||  (addProductController.publishingHouseList?.length ?? 0) < 4 ? null : 150,
                  padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeMedium),
                  child: ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: addProductController.publishingHouseList?.length ?? 0,
                    itemBuilder: (context, index){

                      if(addProductController.publishingHouseList?[index].id == null) return const SizedBox.shrink();

                      return _FilterItem(
                        title: addProductController.publishingHouseList?[index].name,
                        checked: productController.selectedPublishingHouseIds.contains(addProductController.publishingHouseList?[index].id),
                        onTap: () => productController.onChangePublisherIds(addProductController.publishingHouseList![index].id!),
                      );
                    },
                  ),),
              ),
              ...
              _ViewMoreWidget(
                ...
                isActive: (addProductController.publishingHouseList?.length ?? 0) > 3,
              ),
              const SizedBox(height: Dimensions.paddingSizeMedium,),
        ...
        return Column(
          ...
          children: [
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeExtraSmall),
              child: Container(
                height: productController.authorSeeMore || (addProductController.authorsList?.length ?? 0) < 4 ? null : 150,
                padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeMedium),
                child: ListView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  itemCount: addProductController.authorsList?.length ?? 0,
                  itemBuilder: (context, index){

                    if(addProductController.authorsList?[index].id == null) return const SizedBox.shrink();

                    return _FilterItem(
                      title: addProductController.authorsList?[index].name,
                      checked: productController.selectedAuthorIds.contains(addProductController.authorsList?[index].id),
                      onTap: () => productController.onChangeAuthorIds(addProductController.authorsList![index].id!),
                    );
                  },
                ),),
            ),
            ...
            _ViewMoreWidget(
              ...
              isActive: (addProductController.authorsList?.length ?? 0) > 3,
            ),
            const SizedBox(height: Dimensions.paddingSizeMedium,),
...
class _ButtonWidget extends StatelessWidget {
  ...
  List<int> _getSelectedCategoryIds(BuildContext context) {
    return (Provider.of<AddProductController>(context, listen: false).categoryList ?? [])
  ...
  bool _canFilter(ProductModel? productModel, BuildContext context) {
    ...
    final ProductController productController = Provider.of<ProductController>(context, listen: false);
    final AddProductController addProductController = Provider.of<AddProductController>(context, listen: false);
    ...
    return productController.endDate != productModel.endDate ||
        ...
        !_areCategoriesEqual(addProductController.categoryList, productModel.categoryIds) ||
